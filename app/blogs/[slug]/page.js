    ...
    <>
        ...
        <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-zinc-200 dark:border-zinc-800 pb-12 mb-12">
            ...
            <div className="mt-8 md:mt-0 text-left md:text-right shrink-0">
              <span className="block text-sm font-bold uppercase  text-green-600 dark:text-green-500 mb-2">{blog.category}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24">
            ...
            <aside className="lg:col-span-3 lg:col-start-1 order-2 lg:order-1 border-t lg:border-t-0 border-zinc-200 dark:border-zinc-800 pt-12 lg:pt-0">
              <div className="sticky top-32 flex flex-col gap-10">
                {blog.author && (
                  <div>
                    <span className="block text-[10px] font-bold uppercase  text-zinc-400 dark:text-zinc-500 mb-2">Author</span>
                    <span className="block font-display text-xl font-bold uppercase text-zinc-900 dark:text-white ">{blog.author.name}</span>
                    <span className="block text-sm text-zinc-500 dark:text-zinc-400 mt-1">{blog.author.title}</span>
                  </div>
                )}
                <div>
                  <span className="block text-[10px] font-bold uppercase  text-zinc-400 dark:text-zinc-500 mb-2">Read Time</span>
                  <span className="block text-lg font-medium text-zinc-900 dark:text-white">{blog.readTime}</span>
                </div>
                <div>
                  <span className="block text-[10px] font-bold uppercase  text-zinc-400 dark:text-zinc-500 mb-4">Share Article</span>
            ...
            <div className="lg:col-span-8 order-1 lg:order-2">
              <div className="prose prose-xl max-w-none prose-zinc dark:prose-invert prose-p:font-light  prose-p:text-zinc-800 dark:prose-p:text-zinc-300 prose-headings:font-display prose-headings:font-bold prose-headings: prose-a:text-green-600">
                {blog.body.split('\n\n').map((paragraph, index) => {
                  ...
                  if (paragraph.startsWith('- ')) {
                    ...
                    return (
                      <ul key={index} className="list-none space-y-4 my-10 pl-2">
                        {items.map((item, i) => (
                          <li key={i} className="text-xl  text-zinc-800 dark:text-zinc-300 relative pl-8">
      ...
      {relatedPosts.length > 0 && (
        <section className="py-24 bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-200 dark:border-zinc-800">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="font-display text-4xl font-black uppercase  text-zinc-900 dark:text-white mb-12">
              Continue Reading
            </h2>
              ...
              {relatedPosts.map((related) => (
                ...
                >
                  ...
                  <div className="flex flex-col flex-grow">
                    <div className="flex items-center gap-3 mb-4 text-[10px] uppercase  font-bold text-zinc-500 dark:text-zinc-400">
                      <span className="text-green-600 dark:text-green-500">{related.category}</span>
                      <span className="w-1 h-1 rounded-full bg-zinc-300 dark:bg-zinc-700"></span>
                      <span>{related.readTime}</span>
                    </div>
                    <h3 className="font-display text-2xl font-bold  text-zinc-900 dark:text-white group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors ">
                      {related.title}
                    </h3>
                  </div>
                </Link>
