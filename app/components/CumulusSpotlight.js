            ...
            <ul className="space-y-4 mb-10">
              {featureBullets.map((bullet, idx) => (
                <li key={idx} className="flex items-start">
                  ...
                  <span className="text-zinc-700 dark:text-zinc-300 font-light ">{bullet}</span>
                </li>
              ))}
            </ul>

            <Link href={ctaHref} className="inline-flex items-center justify-center rounded-none bg-green-600 px-8 py-4 text-sm font-bold text-white hover:bg-green-700 transition-colors uppercase r">
              {ctaLabel}
            </Link>
          </div>
