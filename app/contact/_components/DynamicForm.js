...
const InputField = ({ label, name, type = "text", required = false, isTextArea = false }) => (
  <div className="flex flex-col">
    <label htmlFor={name} className="text-xs font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest mb-2 flex justify-between">
      {label} {required && <span className="text-zinc-500 dark:text-zinc-400 font-light lowercase normal-case tracking-normal">(required)</span>}
    </label>
    {isTextArea ? (
      <textarea
        id={name}
  ...
  if (isSuccess) {
    return (
      <div className="py-16 text-center border-t border-zinc-200 dark:border-zinc-800" role="alert" aria-live="polite">
        ...
        <h2 className="font-display text-3xl font-black uppercase tracking-tighter text-zinc-900 dark:text-white mb-4">
          Request Received
        </h2>
  ...
  return (
    <div className="w-full" ref={formContainerRef}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12 border-b border-zinc-200 dark:border-zinc-800 pb-12">
        ...
        <div>
          <label htmlFor="pipeline" className="text-xs font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest mb-4 block">
            How did you hear about us? (Pipeline) <span className="text-zinc-500 dark:text-zinc-400 font-light normal-case tracking-normal">(required)</span>
          </label>
          <select
            id="pipeline"
            value={pipeline}
        ...
        <div>
          <label htmlFor="requestType" className="text-xs font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest mb-4 block">
            What can we help you with? <span className="text-zinc-500 dark:text-zinc-400 font-light normal-case tracking-normal">(required)</span>
          </label>
          <select
            id="requestType"
            value={requestType || ""}
              ...
              <fieldset>
                ...
                {requestType === "service" && (
                  <>
                    ...
                    {/* File Upload */}
                    <div className="md:col-span-2 form-anim">
                      <label className="text-xs font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest mb-2 block">
                        Attachments
                      </label>
                    ...
                    <div className="md:col-span-2 form-anim border-t border-zinc-200 dark:border-zinc-800 pt-8 mt-4"><h3 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-widest">Contact Info</h3></div>
                    <div className="form-anim"><InputField label="Contact Name" name="name" required /></div>
                    <div className="form-anim"><InputField label="Phone" name="phone" type="tel" required /></div>
                    <div className="form-anim"><InputField label="Email" name="email" type="email" required /></div>
                  </>
