...
export default function ConversionBand() {
  ...
  return (
    <section ref={containerRef} className="relative bg-zinc-950 py-32 overflow-hidden border-t border-zinc-800">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center z-10">
        <h2 className="cb-anim font-display text-4xl font-black tracking-tighter text-white uppercase mb-10">
          Ready to deploy the Altus II in your fleet?
        </h2>
        <div className="cb-anim flex flex-col sm:flex-row gap-6 justify-center">
          <Link href="/contact" className="inline-flex items-center justify-center rounded-none bg-green-600 px-8 py-4 text-lg font-bold text-white hover:bg-green-700 transition-colors uppercase tracking-wider">
            Speak to an Expert
          </Link>
          <Link href="/contact" className="inline-flex items-center justify-center px-6 py-4 text-sm font-bold text-zinc-300 hover:text-white transition-colors uppercase tracking-wider group">
            Request a Quote <span className="ml-2 group-hover:translate-x-1 transition-transform">→</span>
          </Link>
        </div>
      </div>
    </section>
