    ...
    <section ref={containerRef} className="py-24 bg-zinc-950 text-white overflow-hidden border-t border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <h2 className="catalog-nav font-display text-4xl md:text-5xl font-black  uppercase mb-8">
          ...
          <div className="catalog-nav flex flex-wrap justify-center gap-2">
            {industries.map((tab) => (
              <button
                ...
                className={`px-6 py-3 text-sm font-bold uppercase r rounded-sm transition-all duration-300 ${activeTab === tab.id
            ...
            return (
              ...
              >
                {product.image ? (
                  ...
                  <div className={`absolute inset-0 bg-zinc-900 border-zinc-800 border flex items-center justify-center text-zinc-600 text-xs uppercase `}>
                    Asset Missing
                  </div>
                )}
                ...
                <div className="absolute inset-0 p-8 pr-16 flex flex-col justify-end pointer-events-none">
                  {product.kicker && (
                    <span className="text-[10px] font-bold text-green-500 uppercase  mb-2 pointer-events-auto">
                      {product.kicker}
                    </span>
                  )}
                  <h3 className="font-display text-3xl font-bold uppercase  text-white pointer-events-auto">
                    {product.name}
                  </h3>
                  {product.description && (
